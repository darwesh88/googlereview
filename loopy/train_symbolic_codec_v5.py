...
def parse_args(argv: list[str] | None = None) -> PriorAwareSymbolicCodecConfig:
    parser = argparse.ArgumentParser(description="Train Loopy v5 prior-aware symbolic codec.")
    parser.add_argument("--data-path", default="loopy/example_corpus.txt")
    parser.add_argument("--output-dir", default="loopy/runs/v5_codec")
    ...
    return PriorAwareSymbolicCodecConfig(
...
def create_dataloaders(config: PriorAwareSymbolicCodecConfig) -> tuple[DataLoader, DataLoader]:
...
def main() -> None:
    ...
    print(
        f"Training v5 symbolic codec on {device.type} with {len(train_loader.dataset)} train samples, "
