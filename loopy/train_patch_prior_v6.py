...
from loopy.dynamic_patching import encode_text_to_dynamic_patches
from loopy.grouped_prior_core import HardGroupedPrior
from loopy.symbolic_codec_v6 import SymbolicCodecV6
from loopy.train_patch_prior_v2 import compute_grouped_metrics
...
class V6GroupedPatchDataset(Dataset):
    def __init__(
        self,
...
def parse_args(argv: list[str] | None = None) -> V6PriorConfig:
    parser = argparse.ArgumentParser(description="Train a grouped prior over Loopy v6 dynamic-patch symbols.")
...
def load_codec(run_dir: str, device: torch.device) -> tuple[SymbolicCodecV6, DynamicSymbolicCodecConfig]:
    run_path = Path(run_dir)
    payload = torch.load(run_path / "v6_codec.pt", map_location=device)
    config_dict = payload["config"]
    allowed = {field.name for field in fields(DynamicSymbolicCodecConfig)}
    config = DynamicSymbolicCodecConfig(**{key: value for key, value in config_dict.items() if key in allowed})
...
def build_grouped_dataset(
    samples: list[str],
    codec: SymbolicCodecV6,
    codec_config: DynamicSymbolicCodecConfig,
    device: torch.device,
    batch_size: int,
) -> V6GroupedPatchDataset:
    inputs: list[torch.Tensor] = []
    targets: list[torch.Tensor] = []
    masks: list[torch.Tensor] = []
    byte_counts: list[torch.Tensor] = []
        ...
        for start in range(0, len(samples), batch_size):
            ...
            for sample in batch_samples:
                encoding = encode_text_to_dynamic_patches(
                    sample,
                    codec_config.max_seq_len,
                    codec_config.max_patches,
                    codec_config.max_patch_size,
                    codec_config.min_patch_size,
                    codec_config.patching_mode,
                )
                patch_ids_list.append(encoding.patch_ids)
                patch_mask_list.append(encoding.patch_mask)
                byte_mask_list.append(encoding.byte_mask)
                byte_count_list.append(torch.tensor(encoding.byte_mask, dtype=torch.float32).sum(dim=-1))
            ...
            byte_mask_tensor = torch.tensor(byte_mask_list, dtype=torch.float32, device=device)
            forward = codec(patch_ids_tensor, patch_mask_tensor, byte_mask_tensor)
            ...
            for index in range(len(batch_samples)):
                ...
                byte_counts.append(counts[1:].to(torch.float32))

    return V6GroupedPatchDataset(inputs, targets, masks, byte_counts)


def run_epoch(
    model: HardGroupedPrior,
    loader: DataLoader,
...
def save_artifacts(output_dir: Path, config: V6PriorConfig, model: HardGroupedPrior, best_metrics: dict[str, float]) -> None:
...
def main() -> None:
    ...
    model = HardGroupedPrior(
        [codec_config.sub_codebook_size] * codec_config.num_codebooks,
    ...
    print(
        f"Training v6 grouped dynamic-patch prior on {device.type} with "
