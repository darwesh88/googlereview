...
class V4GroupedPatchDataset(Dataset):
    def __init__(
        self,
...
def parse_args(argv: list[str] | None = None) -> V4PriorConfig:
    parser = argparse.ArgumentParser(description="Train a grouped prior over Loopy v4 patch symbols.")
...
def load_codec(run_dir: str, device: torch.device) -> tuple[SymbolicCodecV4, ContextualSymbolicCodecConfig]:
    run_path = Path(run_dir)
    payload = torch.load(run_path / "v4_codec.pt", map_location=device)
    config_dict = payload["config"]
    config = ContextualSymbolicCodecConfig(
        **{k: v for k, v in config_dict.items() if k in ContextualSymbolicCodecConfig.__dataclass_fields__}
    )
...
def build_grouped_dataset(
    samples: list[str],
    codec: SymbolicCodecV4,
    codec_config: ContextualSymbolicCodecConfig,
    device: torch.device,
    batch_size: int,
) -> V4GroupedPatchDataset:
    inputs: list[torch.Tensor] = []
    targets: list[torch.Tensor] = []
    masks: list[torch.Tensor] = []
    byte_counts: list[torch.Tensor] = []
    ...
    return V4GroupedPatchDataset(inputs, targets, masks, byte_counts)
...
def save_artifacts(output_dir: Path, config: V4PriorConfig, model: GroupedPatchPrior, best_metrics: dict[str, float]) -> None:
...
def main() -> None:
    ...
    print(
        f"Training v4 grouped patch prior on {device.type} with "
