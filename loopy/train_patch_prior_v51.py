...
class V51GroupedPatchDataset(Dataset):
    def __init__(
        self,
...
def parse_args(argv: list[str] | None = None) -> V51PriorConfig:
    parser = argparse.ArgumentParser(description="Train a grouped prior over Loopy v5.1 patch symbols.")
...
def load_codec(run_dir: str, device: torch.device) -> tuple[SymbolicCodecV51, PriorAlignedSymbolicCodecConfig]:
    run_path = Path(run_dir)
    payload = torch.load(run_path / "v51_codec.pt", map_location=device)
    config_dict = payload["config"]
    config = PriorAlignedSymbolicCodecConfig(
        **{
            key: value
            for key, value in config_dict.items()
            if key in PriorAlignedSymbolicCodecConfig.__dataclass_fields__
        }
    )
...
def build_grouped_dataset(
    samples: list[str],
    codec: SymbolicCodecV51,
    codec_config: PriorAlignedSymbolicCodecConfig,
    device: torch.device,
    batch_size: int,
) -> V51GroupedPatchDataset:
    inputs: list[torch.Tensor] = []
    targets: list[torch.Tensor] = []
    masks: list[torch.Tensor] = []
    byte_counts: list[torch.Tensor] = []
    ...
    return V51GroupedPatchDataset(inputs, targets, masks, byte_counts)
...
def save_artifacts(output_dir: Path, config: V51PriorConfig, model: HardGroupedPrior, best_metrics: dict[str, float]) -> None:
...
def main() -> None:
    ...
    print(
        f"Training v5.1 grouped patch prior on {device.type} with "
