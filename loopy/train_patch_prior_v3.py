...
from loopy.symbolic_codec_v3 import SymbolicCodecV3
...
class V3GroupedPatchDataset(Dataset):
    def __init__(
        self,
...
def parse_args(argv: list[str] | None = None) -> V3PriorConfig:
    parser = argparse.ArgumentParser(description="Train a grouped prior over Loopy v3 patch symbols.")
...
def load_codec(run_dir: str, device: torch.device) -> tuple[SymbolicCodecV3, SymbolicCodecConfig]:
    run_path = Path(run_dir)
    payload = torch.load(run_path / "v3_codec.pt", map_location=device)
    config_dict = payload["config"]
    config = SymbolicCodecConfig(**{k: v for k, v in config_dict.items() if k in SymbolicCodecConfig.__dataclass_fields__})
    model = SymbolicCodecV3(config).to(device)
...
def build_grouped_dataset(
    samples: list[str],
    codec: SymbolicCodecV3,
    codec_config: SymbolicCodecConfig,
    device: torch.device,
    batch_size: int,
) -> V3GroupedPatchDataset:
    inputs: list[torch.Tensor] = []
    targets: list[torch.Tensor] = []
    masks: list[torch.Tensor] = []
    byte_counts: list[torch.Tensor] = []
    ...
    return V3GroupedPatchDataset(inputs, targets, masks, byte_counts)
...
def save_artifacts(output_dir: Path, config: V3PriorConfig, model: GroupedPatchPrior, best_metrics: dict[str, float]) -> None:
...
def main() -> None:
    ...
    print(
        f"Training v3 grouped patch prior on {device.type} with "
