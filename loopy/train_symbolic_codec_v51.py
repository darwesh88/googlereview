...
from loopy.symbolic_codec_v51 import SymbolicCodecV51
...
def parse_args(argv: list[str] | None = None) -> PriorAlignedSymbolicCodecConfig:
    parser = argparse.ArgumentParser(description="Train Loopy v5.1 aligned prior-aware symbolic codec.")
    parser.add_argument("--data-path", default="loopy/example_corpus.txt")
    parser.add_argument("--output-dir", default="loopy/runs/v51_codec")
    ...
    parser.add_argument("--prior-group-embed-dim", type=int, default=16)
    parser.add_argument("--prior-hidden-size", type=int, default=128)
    parser.add_argument("--prior-num-layers", type=int, default=2)
    parser.add_argument("--prior-dropout", type=float, default=0.1)
    args = parser.parse_args(argv)
    return PriorAlignedSymbolicCodecConfig(
        ...
        prior_group_embed_dim=args.prior_group_embed_dim,
...
def create_dataloaders(config: PriorAlignedSymbolicCodecConfig) -> tuple[DataLoader, DataLoader]:
...
def run_epoch(
    ...
    with context():
        for batch in loader:
            ...
            totals["prior_match_loss"] += forward.prior_match_loss.item()
            totals["prior_ce_loss"] += forward.prior_ce_loss.item()
            totals["prior_bpb"] += forward.prior_bpb.item()
...
def save_artifacts(
    output_dir: Path,
    config: PriorAlignedSymbolicCodecConfig,
    model: SymbolicCodecV51,
...
def main() -> None:
    ...
    model = SymbolicCodecV51(config).to(device)
    ...
    print(
        f"Training v5.1 symbolic codec on {device.type} with {len(train_loader.dataset)} train samples, "
        ...
        f"prior_weight={config.prior_weight:.4f}"
    )
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        print(
            ...
            f"val_prior_match={val_metrics['prior_match_loss']:.4f} "
            f"val_prior_ce={val_metrics['prior_ce_loss']:.4f} "
            f"val_prior_bpb={val_metrics['prior_bpb']:.4f} "
        ...
        if val_metrics["loss"] < best_val_loss:
            ...
            best_metrics = {
                ...
                "prior_match_loss": val_metrics["prior_match_loss"],
                "prior_ce_loss": val_metrics["prior_ce_loss"],
                "prior_bpb": val_metrics["prior_bpb"],
